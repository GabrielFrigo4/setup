...
flowchart TD
    subgraph QUARTET ["🏛️ O Quarteto de Produtividade"]
        ...
        PROFILE["🎨 2. Profile (Público)<br/>• Dotfiles Declarativos de Usuário<br/>• Editores (Antigravity, VSCode, Zed, Emacs)<br/>• Terminais (Konsole, Windows Terminal)<br/>• Linters, Formatadores e Skills de IA"]
        SHELL["🐚 3. Shell (Público)<br/>• Motor Interativo de Terminal<br/>• Prompts Ultra-rápidos (&lt; 50ms)<br/>• Aliases e Funções de Linha de Comando<br/>• Targets de SO e Contextos"]
        VAULT["🔐 4. Vault (Privado)<br/>• Chaves SSH / PuTTY PPK<br/>• Segredos e Variáveis .env<br/>• Senhas Wi-Fi e Mapeamento de Hosts<br/>• Loaders Multi-Shell (sh, ps1, cmd, nu)"]
    end
    ...
    PROFILE -->|2. sincroniza dotfiles ($HOME)| APPS
    PROFILE -->|2. provê inteligência e skills| CLI
    SHELL -->|3. energiza a sessão| CLI
    VAULT -->|4. injeta credenciais em silêncio| SHELL
    VAULT -->|4. provê chaves para ssh-agent| APPS
```
...
| :--- | :--- | :--- | :--- | :--- | :--- |
| **[Setup](https://github.com/GabrielFrigo4/Setup)** | Público | **O "COMO" (Sistema)**: Provisionamento de pacotes, drivers, containers, hypervisors e serviços base. | Nível SO / Privilegiado (`root` / `sudo` / `ELEVATE` / Admin) | **Cookbook Efêmero (Zero-Clone)**: Execução direta via GitHub web ou `curl \| sh`. | Efêmero / Não requer clone residente |
| **[Profile](https://github.com/GabrielFrigo4/Profile)** | Público | **O "O QUÊ" (Identidade)**: Dotfiles declarativos, editores, terminais, linters, links de sync e skills portáteis de IA. | Nível Usuário (`$HOME`, zero privilégios administrativos) | **Residente Sincronizado**: Cloned no `$HOME`, sincronizado via links simbólicos (`ln -sf`). | `~/.config/profile` ou `~/.profile-repo` |
| **[Shell](https://github.com/GabrielFrigo4/Shell)** | Público | **A "INTERAÇÃO" (Linha de Comando)**: Prompts instantâneos, aliases, cascading de editores e funções POSIX. | Nível Shell / Sessão (disponível para todos os usuários e `root`) | **Residente do Sistema**: Clonado e atualizado via `git pull` contínuo. | `/usr/local/share/shell` ou `~/.shell` |
| **[Vault](https://github.com/GabrielFrigo4/Vault)** | **Privado** | **O "SEGREDO" (Cofre Criptográfico)**: Chaves SSH/PuTTY, tokens de API, credenciais Wi-Fi e endpoints de hosts. | Usuário Restrito (Permissões estritas `0700` e `0600`, zero-leakage) | **Residente Privado**: Clonado exclusivamente em máquinas autorizadas. | `${HOME}/.vault` |
...
sequenceDiagram
    ...
    participant S as 📦 Setup (SO / Root)
    participant P as 🎨 Profile (Dotfiles / $HOME)
    ...
    Dev->>P: 2. Clona Profile e executa sincronizador de dotfiles
    Note over P: Cria symlinks para ~/.config/, editores (Zed, Helix, VSCode) e skills de IA.
    Dev->>Sh: 3. Clona Shell em /usr/local/share/shell e instala (.zshrc / .bashrc)
    Note over Sh: Configura prompts, bibliotecas POSIX, aliases e detecção de SO.
    Dev->>V: 4. Clona Vault em ~/.vault (se ambiente autorizado)
