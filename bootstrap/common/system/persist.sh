...
echo "📦 [Privileges/Persist]: Configurando elevação com persistência de sessão..."
...
if [ "$(uname -s)" = "FreeBSD" ] || command -v pkg > "/dev/null" 2>&1; then
	...
	echo "⚠️ [Privileges/Persist]: Gerenciador de pacotes não suportado automaticamente."
	TARGET_GROUP="wheel"
	DOAS_CONF="/etc/doas.conf"
	SUDOERS_DIR="/etc/sudoers.d"
fi
...
echo "✅ [Privileges/Persist]: Configurado com sucesso para o grupo '${TARGET_GROUP}' (0440, exige senha e salva sessão)!"
