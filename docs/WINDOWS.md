...
## 🏗️ 1. Windows Nativo (`windows/native/`)

A camada nativa provê softwares portáteis e ferramentas de sistema integradas ao Windows:

### A. Gerenciadores de Pacotes

- **Winget (Windows Package Manager):** Gerenciador primário oficial ([`pkg/winget.cmd`](../windows/native/pkg/winget.cmd)) para navegadores, IDEs e utilitários de sistema.
- **Scoop:** Gerenciador voltado para ferramentas de linha de comando portáteis instaladas no espaço do usuário sem privilégios de Administrador ([`pkg/scoop.cmd`](../windows/native/pkg/scoop.cmd)).
- **Chocolatey:** Contingência para pacotes legados ([`pkg/choco.cmd`](../windows/native/pkg/choco.cmd)).

### B. Suite de Engenharia Reversa e Análise de Binários

A pasta [`security/`](../windows/native/security/) concentra ferramentas essenciais para análise de malware, engenharia reversa e auditoria de binários PE/ELF:
...
- [`system.sh`](../windows/msys2/system.sh) — Sistema base, coreutils, Make e shell padrão Bash.
- [`tools.sh`](../windows/msys2/tools.sh) — Git, GitHub CLI, bat, eza, ripgrep, fastfetch e ffmpeg.
- [`editors.sh`](../windows/msys2/editors.sh) — Emacs nativo e Micro.
- [`toolchains.sh`](../windows/msys2/toolchains.sh) — GCC, Clang/LLVM, Ninja, CMake, NASM e toolchain AVR/Embedded.
- [`languages.sh`](../windows/msys2/languages.sh) — Runtimes de Python (+ uv/pip), Lua (+ Luarocks) e Rust (+ toolchains GNU).
- [`graphics.sh`](../windows/msys2/graphics.sh) — Frameworks GUI (GTK3/4, Qt5/6) e gamedev (Raylib, SDL2, SDL3, Vulkan, GLFW).
- [`libraries.sh`](../windows/msys2/libraries.sh) — OpenSSL, Zstd, SQLite3, Libuv e Tree-sitter.
